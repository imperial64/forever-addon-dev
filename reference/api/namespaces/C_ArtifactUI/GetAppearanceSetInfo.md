<!-- GENERATED from Blizzard_APIDocumentation, client build 69913. Do not edit; edit the generator. -->

# C_ArtifactUI.GetAppearanceSetInfo

```lua
artifactAppearanceSetID, appearanceSetName, appearanceSetDescription, numAppearances = C_ArtifactUI.GetAppearanceSetInfo(appearanceSetIndex)
```

**Arguments**

| # | Name | Type | Nilable | Default |
|---|---|---|---|---|
| 1 | `appearanceSetIndex` | `number` | no |  |

**Returns**

| # | Name | Type | Nilable | Default |
|---|---|---|---|---|
| 1 | `artifactAppearanceSetID` | `number` | no |  |
| 2 | `appearanceSetName` | `string` | no |  |
| 3 | `appearanceSetDescription` | `string` | no |  |
| 4 | `numAppearances` | `number` | no |  |

Blizzard's own rendering: `C_ArtifactUI.GetAppearanceSetInfo(appearanceSetIndex)`

System: `ArtifactUI` · Namespace: `C_ArtifactUI`

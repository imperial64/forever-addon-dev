<!-- GENERATED from Blizzard_APIDocumentation, client build 69913. Do not edit; edit the generator. -->

# C_ContentTracking.GetEncounterTrackingInfo

```lua
trackingInfo = C_ContentTracking.GetEncounterTrackingInfo(journalEncounterID)
```

**Arguments**

| # | Name | Type | Nilable | Default |
|---|---|---|---|---|
| 1 | `journalEncounterID` | `number` | no |  |

**Returns**

| # | Name | Type | Nilable | Default |
|---|---|---|---|---|
| 1 | `trackingInfo` | `EncounterTrackingInfo` | no |  |

Blizzard's own rendering: `C_ContentTracking.GetEncounterTrackingInfo(journalEncounterID)`

System: `ContentTracking` · Namespace: `C_ContentTracking`

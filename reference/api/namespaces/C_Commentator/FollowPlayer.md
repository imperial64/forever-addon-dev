<!-- GENERATED from Blizzard_APIDocumentation, client build unknown. Do not edit; edit the generator. -->

# C_Commentator.FollowPlayer

```lua
C_Commentator.FollowPlayer(factionIndex, playerIndex, forceInstantTransition)
```

**Arguments**

| # | Name | Type | Nilable | Default |
|---|---|---|---|---|
| 1 | `factionIndex` | `luaIndex` | no |  |
| 2 | `playerIndex` | `luaIndex` | no |  |
| 3 | `forceInstantTransition` | `bool` | yes |  |

**Returns**

_None._

Blizzard's own rendering: `C_Commentator.FollowPlayer(factionIndex, playerIndex, optional forceInstantTransition)`

System: `CommentatorFrame` · Namespace: `C_Commentator`

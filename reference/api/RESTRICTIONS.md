...
Brightness, Contrast and Gamma carry no lock flags - GetCVarInfo reports isLockedFromUser, isSecure and isReadOnly all false. Writes land through both C_CVar.SetCVar and the undocumented global SetCVar, identically, and read back exactly. A write is a cheap live post-process, not a device restart: a four-second sweep writing every frame from OnUpdate managed 442 writes over 443 frames at 111 fps, worst frame gap 31 ms. That is a ceiling rather than a cost; the per-call figures are in research/costs.yaml under cvar-write, and a write is cheap in time but allocates ~822 bytes, which is the cost that actually constrains a per-frame design. Brightness and Contrast are on a 0-100 scale (default 50), Gamma centres on 1.0. Measured in maximized windowed mode at 1920x1080, so this is not fullscreen-only. MEASURED IN COMBAT on 2026-09-20 and permitted. The same instrument ran out of combat and mid-fight one pull apart: writes landed and read back identically, the lock flags stayed false, and no ADDON_ACTION_BLOCKED or ADDON_ACTION_FORBIDDEN was captured in either state. That refusal count is a captured event count rather than a pcall result, which is the distinction this client requires. A per-frame sweep driving BOTH Brightness and Contrast managed 1742 writes over 871 frames in combat at 290 fps, worst frame gap 9 ms - five times the write rate of the out-of-combat ramp above. This entry previously said combat was unmeasured and advised freezing on PLAYER_REGEN_DISABLED; that caveat is withdrawn. NOT measured: whether this extends to CVars other than Brightness and Contrast, or to instanced rather than open-world combat. /fprobe video in combat has still not been run - the combat result came from a third-party instrument, not from this repo's probe.

**Workaround.** None needed for combat. If you drive one of these from OnUpdate, the cost to watch is allocation rather than frame time - see cvar-write in research/costs.yaml.

_Evidence: §P.22, §P.29 in `research/findings.md`._
